...
def getsession():
    ...
    print("START")
    #if 'user' in session:
    session['STATE'] = "WELCOME_ASK_NAME"
    session['cust_name'] = ""
    session['is_nickname'] = ""
    session['cust_nickname'] = ""
    session['cust_birthday'] = ""
    session['is_time_for_more'] = ""
    session['cust_phone'] = ""
    session['cust_color'] = ""
    session['cust_type_of_salon'] = ""
    session['is_reservation_now'] = ""
    session['cust_date_obj'] = ""
    session['cust_service_id'] = ""
    session['cust_avail_msg'] = ""
    session['cust_avail_display_options'] = []
    session['cust_avail_option_list'] = []
    session['cust_responses'] = {}
    session['return_list_of_dicts'] = []
    session['duration'] = 2
    print("END")
    print("Session", session)
    #return "yes"
...
def chat():
    ...
    #global cust_name
    #global is_nickname
    #global session['cust_nickname']
    #global session['cust_first_name']
    #global cust_birthday
    #global session['is_time_for_more']
    #:global session['session['cust_phone']']
    #global session['cust_color']
    #global session['cust_type_of_salon']
    #global session['is_reservation_now']
    #global session['cust_date_obj']
    #global session['cust_service_id']
    #global session['cust_avail_msg']
    #global session['cust_avail_display_options']
    #global session['cust_avail_option_list']
    #global session['cust_responses']
    #global session['return_list_of_dicts']
    ...
    if session['STATE'] == "WELCOME_ASK_NAME":
        print("1")
        out_msg = welcome_ask_name()
        session['STATE'] = "NAME_ASKED"
        out_dict = {"type" : "input", "question": out_msg, "answer": 0}
        session['return_list_of_dicts'].append(out_dict)
        #out_json = json.dumps(out_dict,ensure_ascii= False)
        out_json = json.dumps(session['return_list_of_dicts'],ensure_ascii= False)
        print(session["STATE"])
        
        return out_json
    
    
    if session['STATE'] == "NAME_ASKED":
        print("2")
        session['cust_name'] = inp_msg
        name_st,out_msg = check_name(session['cust_name'])
        if name_st == 1:
            session['cust_responses']["name"] = session['cust_name']
            session['return_list_of_dicts'][-1]["answer"] = session['cust_name']
            session['return_list_of_dicts'][-1]["type"] = "text"
            session['STATE'] = "IS_NICKNAME"
        else:
            return out_msg


    if session['STATE'] == "IS_NICKNAME":
        out_msg,option_list = is_nickname()
        session['STATE'] = "IS_NICKNAME_ASKED"
    ...
    if session['STATE'] == "IS_NICKNAME_ASKED":
        ...
        if is_nickname_status == 1:
            session['cust_responses']["is_nickname"] = is_nickname_response
            session['return_list_of_dicts'][-1]["answer"] = is_nickname_response
            session['return_list_of_dicts'][-1]["type"] = "text"
            if is_nickname_response == "yes":
                session['STATE'] = "ASK_NICKNAME"
            elif is_nickname_response == "no":
                session['STATE'] = "ASK_BIRTHDAY"
        else:
            return out_msg


    if session['STATE'] == "ASK_NICKNAME":
        out_msg = ask_nickname()
        session['STATE'] = "NICKNAME_ASKED"
        out_dict = {"type" : "input", "question": out_msg, "answer": 0}
        session['return_list_of_dicts'].append(out_dict)
        out_json = json.dumps(session['return_list_of_dicts'],ensure_ascii= False)
        return out_json

    if session['STATE'] == "NICKNAME_ASKED":
        session['cust_nickname'] = inp_msg
        nickname_status,out_msg = check_name(session['cust_nickname'])
        if nickname_status == 1:
            session['cust_responses']["nickname"] = session['cust_nickname']
            session['return_list_of_dicts'][-1]["answer"] = session['cust_nickname']
            session['return_list_of_dicts'][-1]["type"] = "text"
            session['STATE'] = "ASK_BIRTHDAY"
        else:
            return out_msg
    ...
    if session['STATE'] == "ASK_BIRTHDAY":
        out_msg = ask_birthday()
        session['STATE'] = "BIRTHDAY_ASKED"
        out_dict = {"type" : "input", "question": out_msg, "answer": 0}
        session['return_list_of_dicts'].append(out_dict)
        out_json = json.dumps(session['return_list_of_dicts'],ensure_ascii= False)
        return out_json

    if session['STATE'] == "BIRTHDAY_ASKED":
        session['cust_birthday'] = inp_msg
        birthday_obj,birthday_status,out_msg = check_date(session['cust_birthday'])
        birthday_str = str(birthday_obj)
        if birthday_status == 1:
            session['cust_responses']["birthday"] = birthday_str
            session['return_list_of_dicts'][-1]["answer"] = birthday_str
            session['return_list_of_dicts'][-1]["type"] = "text"
            session['STATE'] = "IS_TIME_FOR_MORE"
        else:
            return out_msg



    if session['STATE'] == "IS_TIME_FOR_MORE":
        out_msg,option_list = is_time_for_more()
        session['STATE'] = "IS_TIME_FOR_MORE_ASKED"
        out_dict = {"type" : "option", "question": out_msg, "option_list": option_list, "answer": 0}
        session['return_list_of_dicts'].append(out_dict)
        out_json = json.dumps(session['return_list_of_dicts'],ensure_ascii= False)
        return out_json
    ...
    if session['STATE'] == "IS_TIME_FOR_MORE_ASKED":
        ...
        if is_time_for_more_status == 1:
            ...
            session['cust_responses']['is_time_for_more'] = is_time_for_more_response
            session['return_list_of_dicts'][-1]["answer"] = is_time_for_more_response
            session['return_list_of_dicts'][-1]["type"] = "text"
            if is_time_for_more_response == "yes":
                session['STATE'] = "ASK_PHONE"
            elif is_time_for_more_response == "no":
                session['STATE'] = "IS_RESERVATION_NOW"
        else:
            return out_msg
    ...
    if session['STATE'] == "ASK_PHONE":
        out_msg = ask_phone()
        session['STATE'] = "PHONE_ASKED"
        out_dict = {"type" : "input", "question": out_msg, "answer": 0}
        session['return_list_of_dicts'].append(out_dict)
        out_json = json.dumps(session['return_list_of_dicts'],ensure_ascii= False)
        return out_json

    if session['STATE'] == "PHONE_ASKED":
        session['cust_phone'] = inp_msg
        phone_status,out_msg = check_phone(session['cust_phone'])
        if phone_status == 1:
            session['cust_responses']["phone"] = session['cust_phone']
            session['return_list_of_dicts'][-1]["answer"] = session['cust_phone']
            session['return_list_of_dicts'][-1]["type"] = "text"
            session['STATE'] = "ASK_COLOR"
        else:
            return out_msg
    ...
    if session['STATE'] == "ASK_COLOR":
        out_msg = ask_color()
        session['STATE'] = "COLOR_ASKED"
        out_dict = {"type" : "input", "question": out_msg, "answer": 0}
        session['return_list_of_dicts'].append(out_dict)
        out_json = json.dumps(session['return_list_of_dicts'],ensure_ascii= False)
        return out_json

    if session['STATE'] == "COLOR_ASKED":
        session['cust_color'] = inp_msg
        
        color_status,out_msg = check_color(session['cust_color'])
        if color_status == 1:
            session['cust_responses']["color"] = session['cust_color']
            session['return_list_of_dicts'][-1]["answer"] = session['cust_color']
            session['return_list_of_dicts'][-1]["type"] = "text"
            session['STATE'] = "ASK_TYPE_OF_SALON"
        else:
            return out_msg


    if session['STATE'] == "ASK_TYPE_OF_SALON":
        out_msg,option_list = ask_type_of_salon()
        session['STATE'] = "TYPE_OF_SALON_ASKED"
        out_dict = {"type" : "option", "question": out_msg, "option_list": option_list, "answer": 0}
        session['return_list_of_dicts'].append(out_dict)
        out_json = json.dumps(session['return_list_of_dicts'],ensure_ascii= False)
        return out_json

    if session['STATE'] == "TYPE_OF_SALON_ASKED":
        cust_type_of_salon_menu_int = inp_msg
        
        cust_type_of_salon_status,session['cust_type_of_salon'],out_msg = check_cust_type_of_salon(cust_type_of_salon_menu_int)
        
        if cust_type_of_salon_status == 1:
            session['cust_responses']["type_of_salon"] = session['cust_type_of_salon']
            session['return_list_of_dicts'][-1]["answer"] = session['cust_type_of_salon']
            session['return_list_of_dicts'][-1]["type"] = "text"
    ...
    if session['STATE'] == "IS_RESERVATION_NOW":
        out_msg,option_list = is_reservation_now()
        session['STATE'] = "IS_RESERVATION_NOW_ASKED"
        out_dict = {"type" : "option", "question": out_msg, "option_list": option_list, "answer": 0}
        session['return_list_of_dicts'].append(out_dict)
        out_json = json.dumps(session['return_list_of_dicts'],ensure_ascii= False)
        return out_json
    ...
    if session['STATE'] == "IS_RESERVATION_NOW_ASKED":
        ...
        if is_reservation_now_status == 1:
            session['cust_responses']["is_reservation_now"] = is_reservation_now_response
            session['return_list_of_dicts'][-1]["answer"] = is_reservation_now_response
            session['return_list_of_dicts'][-1]["type"] = "text"
            if is_reservation_now_response == "yes":
                session['STATE'] = "ASK_DATE"
            elif is_reservation_now_response == "no":
                session['STATE'] = "GOOD_BYE"
        else:
            return out_msg
    ...
    if session['STATE'] == "ASK_DATE":
        out_msg = ask_date()
        session['STATE'] = "DATE_ASKED"
        out_dict = {"type" : "input", "question": out_msg, "answer": 0}
        session['return_list_of_dicts'].append(out_dict)
        out_json = json.dumps(session['return_list_of_dicts'],ensure_ascii= False)
        return out_json

    if session['STATE'] == "DATE_ASKED":
        cust_date = inp_msg
        session['cust_date_obj'],cust_date_status,out_msg = check_date(cust_date)
        cust_date_str = str(session['cust_date_obj'])
        if cust_date_status == 1:
            session['cust_responses']["date"] = cust_date_str
            session['return_list_of_dicts'][-1]["answer"] = cust_date_str
            session['return_list_of_dicts'][-1]["type"] = "text"
    ...
    if session['STATE'] == "ASK_SERVICE":
        ...
        session['STATE'] = "SERVICE_ASKED"
        out_dict = {"type" : "option", "question": out_msg, "option_list": option_list,"answer": 0}
        session['return_list_of_dicts'].append(out_dict)
        out_json = json.dumps(session['return_list_of_dicts'],ensure_ascii= False)
        return out_json

    if session['STATE'] == "SERVICE_ASKED":
        
        cust_service_menu_int = inp_msg
        cust_service_status, session['cust_service_id'], out_msg = check_service(cust_service_menu_int)
        
        if cust_service_status == 1:
            session['cust_responses']["service"] = session['cust_service_id']
            session['return_list_of_dicts'][-1]["answer"] = session['cust_service_id']
            session['return_list_of_dicts'][-1]["type"] = "text"
            relevant_employee_list = find_employees_for_service(session['cust_service_id'])
            avail_emp_dict = check_new_availability(session['cust_date_obj'],relevant_employee_list,session['duration'])
            emp_name_dict = find_employee_name()
            session['cust_avail_msg'], session['cust_avail_display_options'] ,session['cust_avail_option_list'] = convert_avail_dict_to_display_options(avail_emp_dict,emp_name_dict)
            
            # Cust Avail Display Options is in "short" format
            # Cust Avail Option List is in "full" format
            
            session['STATE'] = "SHOW_AVAIL_OPTIONS"
        else:
            return out_msg

    if session['STATE'] == "SHOW_AVAIL_OPTIONS":
        out_msg = show_avail_options()
        session['STATE'] = "AVAIL_OPTIONS_SHOWN"
        out_dict = {"type" : "option", "question": out_msg, "option_list": session['cust_avail_option_list'],"answer": 0}
        session['return_list_of_dicts'].append(out_dict)
        out_json = json.dumps(session['return_list_of_dicts'],ensure_ascii= False)
        return out_json

    if session['STATE'] == "AVAIL_OPTIONS_SHOWN":
        cust_avail_options_menu_int = inp_msg
        cust_avail_options_status, cust_selected_option, out_msg = check_avail_options(cust_avail_options_menu_int,session['cust_avail_display_options'])
        
        if cust_avail_options_status == 1:
            session['cust_responses']["avail_options"] = cust_selected_option
            session['return_list_of_dicts'][-1]["answer"] = cust_selected_option
            session['return_list_of_dicts'][-1]["type"] = "text"
            
            session['STATE'] = "IS_CONFIRM"
        else:
            return out_msg

    if session['STATE'] == "IS_CONFIRM":
        out_msg, option_list = is_confirm()
        session['STATE'] = "IS_CONFIRM_ASKED"
        out_dict = {"type" : "option", "question": out_msg, "option_list": option_list,"answer": 0}
        session['return_list_of_dicts'].append(out_dict)
        out_json = json.dumps(session['return_list_of_dicts'],ensure_ascii= False)
        return out_json

    if session['STATE'] == "IS_CONFIRM_ASKED":
        ...
        if cust_is_confirm_status == 1:
            session['cust_responses']["is_confirm"] = cust_is_confirm_response
            session['return_list_of_dicts'][-1]["answer"] = cust_is_confirm_response
            session['return_list_of_dicts'][-1]["type"] = "text"

            if cust_is_confirm_response == "yes":
                session['STATE'] = "ENTER_DB"
        else:
            return out_msg
    
    if session['STATE'] == "ENTER_DB":
        print ("inside enter db")
        cust_name = session["cust_responses"]["name"]
        cust_is_nickname = session["cust_responses"]["is_nickname"]
        cust_nickname = session["cust_responses"]["nickname"]
        cust_birthday = session["cust_responses"]["birthday"]
        cust_is_time_for_more = session["cust_responses"]["is_time_for_more"]
        cust_phone = session["cust_responses"]["phone"]
        cust_color = session["cust_responses"]["color"]
        cust_type_of_salon = session["cust_responses"]["type_of_salon"]
        cust_is_reservation_now = session["cust_responses"]["is_reservation_now"]
        cust_res_date = session["cust_responses"]["date"]
        cust_slot = session["cust_responses"]["avail_options"][2]
        cust_res_time = slot_list[cust_slot]
        cust_service_id = session["cust_responses"]["service"]
        cust_emp_id = session["cust_responses"]["avail_options"][1]
        cust_is_confirm = session["cust_responses"]["is_confirm"]
        cust_last_state = session["STATE"]
...
if __name__ == '__main__':
    ...
    app.run(debug=True,port=12345,host='0.0.0.0',ssl_context=context)   
